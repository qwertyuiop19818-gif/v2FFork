...
object HttpUtil {
    ...
    /**
     * Checks if the response is gzipped based on Content-Encoding header
     */
    private fun isGzipResponse(conn: HttpURLConnection): Boolean {
        val contentEncoding = conn.getHeaderField("Content-Encoding")
        return contentEncoding?.equals("gzip", ignoreCase = true) == true
    }

    /**
     * Reads from connection with automatic gzip decompression if needed
     */
    private fun readResponse(conn: HttpURLConnection): String {
        val inputStream = if (isGzipResponse(conn)) {
    ...
    /**
     * Reads from error stream with automatic gzip decompression if needed
     */
        ...
        return try {
            ...
            if (errorStream != null) {
                if (isGzipResponse(conn)) {
                    GZIPInputStream(errorStream).bufferedReader().use { it.readText() }
                } else {
    ...
    fun getUrlContentWithUserAgent(url: String?, userAgent: String?, timeout: Int = 15000, httpPort: Int = 0): String {
        ...
        while (redirects++ < maxRedirects) {
            ...
            // Устанавливаем User-Agent только если передан конкретный userAgent
    ...
    fun createProxyConnection(
        ...
        try {
            ...
            // Добавляем статичные заголовки для всех запросов
            conn.setRequestProperty("User-Agent", "Happ/3.10.0")
            conn.setRequestProperty("x-device-locale", "ru")
            conn.setRequestProperty("x-hwid", "8f42b9a1c3d7e056")
            conn.setRequestProperty("x-device-os", "Android")
            conn.setRequestProperty("x-ver-os", "14")
            conn.setRequestProperty("x-device-model", "KG5n")
            // Заменяем "identity" на "gzip" для поддержки сжатия по умолчанию
            conn.setRequestProperty("accept-encoding", "gzip")
