...
fun importBatchConfig(server: String?, subid: String, append: Boolean): Pair<Int, Int> {
    ...
    private fun parseConfig(
        ...
        try {
            ...
            if (str.startsWith(EConfigType.VLESS.protocolScheme)) {
                ...
                if (hiddenSubResult != null) {
                    // This is a hidden subscription, not a server config
                    return null
                }
            }
    ...
    /**
     * Parses a hidden subscription from a VLESS link with data parameter.
     *
     * @param str The VLESS link string.
     * @param subid The subscription ID.
    ...
    private fun parseHiddenSubscriptionFromVless(str: String, subid: String): Boolean? {
        ...
        try {
            ...
            val fragmentStart = str.indexOf('#')
            val queryString = if (fragmentStart != -1) {
                str.substring(queryStart + 1, fragmentStart)
            } else {
                str.substring(queryStart + 1)
            }

            // Parse query parameters
            val params = queryString.split('&')
            val dataParam = params.firstOrNull { it.startsWith("data=") }?.substring(5)

            if (dataParam != null) {
                // URL decode the data parameter
                val decodedData = URLDecoder.decode(dataParam, StandardCharsets.UTF_8.toString())
...
if (base64Decoded.contains("://")) {
    ...
    if (Utils.isValidSubUrl(base64Decoded)) {
        ...
        if (config != null) {
            ...
            LogUtil.e(AppConfig.TAG, "Failed to parse hidden subscription from VLESS link", e)
        }
        return null
    }
